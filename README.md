...
一个 Streamlit 中文穿搭推荐应用。V0.4 在可选的阿里云百炼千问视觉分析之上，新增经用户确认后的图片偏好排序。
...
6. `recommender.py` 读取并校验 `data/products.csv`，先按预算、尺码、场景、原风格和排除颜色生成合法三件套组合。
7. 普通模式沿用原基础匹配分；图片偏好模式在基础分上增加有上限的颜色协调与风格命中软分，并生成真实规则理由。
8. 合法组合按最终分确定性排序，再进行多样性贪心重排；Streamlit 展示最多三套结果。
